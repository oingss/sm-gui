...
/// 应用此节点/测试延迟/上移/下移/编辑/删除）；顶部订阅管理入口；
/// 底部 FAB 导入节点。移动端测延迟用 TCP 直连（无内核二进制）。
library;
...
class _NodesPageState extends ConsumerState<NodesPage> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
            groups == null || nodes == null
                ? '节点'
                : '${groups.length} 个分组 / ${nodes.length} 个节点',
        ...
        actions: [
          TextButton.icon(
            ...
            label: const Text('订阅管理', style: TextStyle(fontSize: 13)),
            style: TextButton.styleFrom(foregroundColor: SmPalette.accent),
          ),
        ],
      ),
                    ...
                    itemBuilder: (context, i) {
                      ...
                      return _GroupSection(
                        ...
                        onNodeMenu: (node) {
                          ...
                          _showNodeMenu(node, node.id == appliedId, idx,
                              groupNodes.length);
                        },
                      );
                    },
